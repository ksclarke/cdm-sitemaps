...
function createSitemap($alias, $name, $http, $service) {
	...
	while ($total > 500 || $total == -1) {
		if ($total == -1) {
			Zend_Registry::get('logger')->log('Starting sitemap for ' . $name, Zend_Log::DEBUG);
		}
		...
		//Zend_Registry::get('logger')->log($start . ' ' . $total, Zend_Log::DEBUG);
		...
		foreach ($records as $record) {
			...
			//Zend_Registry::get('logger')->log($id . ' ' . $mod, Zend_Log::DEBUG);
		}
	}
...
https://server####.contentdm.oclc.org/dmwebservices/index.php?q=dmGetItemInfo/alias/uid/xml
